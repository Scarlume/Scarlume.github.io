...
const CategoryNodeView = ({ categoryName, categoryNode, categoryCounts, level = 0 }: Props) => {
  ...
  return (
    <div className={`${styles.categoryGroup} ${styles[`level${level}`]}`}>
      {/* 类别标题 */}
      <div
        className={isMainCategory ? styles.mainCategory : styles.subCategory}
        data-category={categoryName}
      >
        <div className={styles.categoryHeader}>
          {(hasChildren || hasPosts) && (
            <button className={styles.expandBtn} aria-label="展开/收起" onClick={handleExpandClick}>
              <svg
          ...
          <a
            href={`/categories/${encodeURIComponent(categoryNode.path)}`}
      ...
      {hasChildren && isExpanded && (
        <div className={styles.subcategories} data-parent={categoryName}>
      ...
      {/* 当前类别下的直接文章 */}
      {hasPosts && isExpanded && (
        <div className={`${styles.postsList} ${isMainCategory ? styles.mainPosts : ''}`}>
